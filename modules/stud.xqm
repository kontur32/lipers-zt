...
(:~
 : Возвращает количество пропусков по всем предметам
 : @param  $data  store as binary
 : @param  $ученик   error string
 : @return page
 :)
declare 
  %public
function stud:количествоПропусковПоПредметам( $данные, $ученик, $начальнаДата, $конечнаяДата ){
  let $записиПоПредметам := 
    stud:записиПоВсемПредметамЗаПериод(
      $данные, $ученик, xs:date( '2020-03-30' ), xs:date( '2020-05-30' )
    )
